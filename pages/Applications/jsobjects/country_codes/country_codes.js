export default {
	countries: [
		"アンドラ公国(ANDORRA)", 
		"アラブ首長国連邦(UNITED ARAB EMIRATES)", 
		"アフガニスタン・イスラム国(AFGHANISTAN)", 
		"アンチグア・バーブーダ(ANTIGUA AND BARBUDA)", 
		"アンギラ(ANGUILLA)", 
		"アルバニア共和国(ALBANIA)", 
		"アルメニア共和国(ARMENIA)", 
		"オランダ領アンチル(NETHERLANDS ANTILLES)", 
		"アンゴラ共和国(ANGOLA)", 
		"南極(ANTARCTICA)", 
		"アルゼンチン共和国(ARGENTINA)", 
		"米領サモア(AMERICAN SAMOA)", 
		"オーストリア共和国(AUSTRIA)", 
		"オーストラリア(AUSTRALIA)", 
		"アルバ(ARUBA)", 
		"アゼルバイジャン共和国(AZERBAIJAN)", 
		"(ボスニア・ヘルツェゴビナ共和国)(BOSNIA AND HERZEGOVINA)", 
		"バルバドス(BARBADOS)", 
		"バングラデシュ人民共和国(BANGLADESH)", 
		"ベルギー王国(BELGIUM)", 
		"ブルキナファソ(BURKINA FASO)", 
		"ブルガリア共和国(BULGARIA)", 
		"バーレーン国(BAHRAIN)", 
		"ブルンジ共和国(BURUNDI)", 
		"ベナン共和国(BENIN)", 
		"バーミューダ諸島(BERMUDA)", 
		"ブルネイ・ダルサラーム国(BRUNEI DARUSSALAM)", 
		"ボリビア共和国(BOLIVIA)", 
		"ブラジル連邦共和国(BRAZIL)", 
		"バハマ国(BAHAMAS)", 
		"ブータン王国(BHUTAN)", 
		"ブーベ島(BOUVET ISLAND)", 
		"ボツワナ共和国(BOTSWANA)", 
		"ベラルーシ共和国(BELARUS)", 
		"ベリーズ(BELIZE)", 
		"カナダ(CANADA)", 
		"ココス諸島(COCOS (KEELING) ISLANDS)", 
		"中央アフリカ共和国(CENTRAL AFRICAN REPUBLIC)", 
		"コンゴ共和国(CONGO)", 
		"スイス連邦(SWITZERLAND)", 
		"コートジボアール共和国(COTE D'IVOIRE)", 
		"クック諸島(COOK ISLANDS)", 
		"チリ共和国(CHILE)", 
		"カメルーン共和国(CAMEROON)", 
		"中華人民共和国(CHINA)", 
		"コロンビア共和国(COLOMBIA)", 
		"コスタリカ共和国(COSTA RICA)", 
		"キューバ共和国(CUBA)", 
		"カーボベルデ共和国(CAPE VERDE)", 
		"クリスマス島(CHRISTMAS ISLAND)", 
		"キプロス共和国(CYPRUS)", 
		"チェコ共和国(CZECH REPUBLIC)", 
		"ドイツ連邦共和国(GERMANY)", 
		"ジブチ共和国(DJIBOUTI)", 
		"デンマーク王国(DENMARK)", 
		"ドミニカ国(DOMINICA)", 
		"ドミニカ共和国(DOMINICAN REPUBLIC)", 
		"アルジェリア民主人民共和国(ALGERIA)", 
		"エクアドル共和国(ECUADOR)", 
		"エストニア共和国(ESTONIA)", 
		"エジプト・アラブ共和国(EGYPT)", 
		"西サハラ(WESTERN SAHARA)", 
		"エリトリア(ERITREA)", 
		"スペイン(SPAIN)", 
		"エチオピア(ETHIOPIA)", 
		"フィンランド共和国(FINLAND)", 
		"フィジー共和国(FIJI)", 
		"フォークランド(マルビナス)諸島(FALKLAND ISLAND (MALVINAS))", 
		"ミクロネシア連邦(MICRONESIA (FEDERATED STATES OF))", 
		"フェロー諸島(FAROE ISLANDS)", 
		"フランス共和国(FRANCE)", 
		"フランス本国(FRANCE, METROPOLITAN)", 
		"ガボン共和国(GABON)", 
		"グレートブリテン及び北部アイルランド連合王国(英国)(UNITED KINGDOM)", 
		"グレナダ(GRENADA)", 
		"グルジア共和国(GEORGIA)", 
		"仏領ギアナ(FRENCH GUIANA)", 
		"ガーナ共和国(GHANA)", 
		"ジブラルタル(GIBRALTAR)", 
		"グリーンランド(GREENLAND)", 
		"ガンビア共和国(GAMBIA)", 
		"ギニア共和国(GUINEA)", 
		"グアドループ島(GUADELOUPE)", 
		"赤道ギニア共和国(EQUATORIAL GUINEA)", 
		"ギリシア共和国(GREECE)", 
		"南ジョージア島・南サンドイッチ諸島(SOUTH GEORGIA AND THE SOUTH SANDWICH ISLANDS)", 
		"グアテマラ共和国(GUATEMALA)", 
		"グアム(GUAM)", 
		"ギニアビサオ共和国(GUINEA-BISSAU)", 
		"ガイアナ協同共和国(GUYANA)", 
		"ホンコン (香港)(HONG KONG)", 
		"ヘアド島・マクドナルド諸島(HEARD ISLAND AND MCDONALD ISLANDS)", 
		"ホンジュラス共和国(HONDURAS)", 
		"クロアチア共和国(CROATIA)", 
		"ハイチ共和国(HAITI)", 
		"ハンガリー共和国(HUNGARY)", 
		"インドネシア共和国(INDONESIA)", 
		"アイルランド(IrelAND)", 
		"イスラエル国(ISRAEL)", 
		"インド(INDIA)", 
		"英領インド洋地域(BRITISH INDIAN OCEAN TERRITORY)", 
		"イラク共和国(IRAQ)", 
		"イラン・イスラム共和国(IRAN (ISLAMIC REPUBLIC OF))", 
		"アイスランド共和国(ICELAND)", 
		"イタリア共和国(ITALY)", 
		"ジャマイカ(JAMAICA)", 
		"ヨルダン・ハシミテ王国(JORDAN)", 
		"日本国(JAPAN)", 
		"ケニア共和国(KENYA)", 
		"キルギスタン共和国(KYRGYZSTAN)", 
		"カンボディア王国(CAMBODIA)", 
		"キリバス共和国(KIRIBATI)", 
		"コモロ・イスラム連邦共和国(COMOROS)", 
		"セントクリストファー・ネイビス(SAINT KITTS AND NEVIS)", 
		"(北朝鮮=朝鮮民主主義人民共和国)(KOREA, DEMOCRATIC PEOPLE'S REPUBLIC OF)", 
		"大韓民国(KOREA, REPUBLIC OF)", 
		"クウェート国(KUWAIT)", 
		"ケイマン諸島(CAYMAN ISLANDS)", 
		"カザフスタン共和国(KAZAKHSTAN)", 
		"ラオス人民民主共和国(LAO PEOPLE'S DEMOCRATIC REPUBLIC)", 
		"レバノン共和国(LEBANON)", 
		"セントルシア(SAINT LUCIA)", 
		"リヒテンシュタイン公国(LIECHTENSTEIN)", 
		"スリランカ民主社会主義共和国(SRI LANKA)", 
		"リベリア共和国(LIBERIA)", 
		"レソト王国(LESOTHO)", 
		"リトアニア共和国(LITHUANIA)", 
		"ルクセンブルク大公国(LUXEMBOURG)", 
		"ラトビア共和国(LATVIA)", 
		"社会主義人民リビア・アラブ国(LIBYAN ARAB JAMAHIRIYA)", 
		"モロッコ王国(MOROCCO)", 
		"モナコ公国(MONACO)", 
		"モルドバ共和国(MOLDOVA, REPUBLIC OF)", 
		"マダガスカル共和国(MADAGASCAR)", 
		"マーシャル諸島共和国(MARSHALL ISLANDS)", 
		"マリ共和国(MALI)", 
		"ミャンマー連邦(MYANMAR)", 
		"モンゴル国(MONGOLIA)", 
		"マカオ(澳門)(MACAU)", 
		"北マリアナ諸島(NORTHERN MARIANA ISLANDS)", 
		"マルチニーク島(MARTINIQUE)", 
		"モーリタニア・イスラム共和国(MAURITANIA)", 
		"モントセラト(MONTSERRAT)", 
		"マルタ共和国(MALTA)", 
		"モーリシャス共和国(MAURITIUS)", 
		"モルジブ共和国(MALDIVES)", 
		"マラウイ共和国(MALAWI)", 
		"メキシコ合衆国(MEXICO)", 
		"マレーシア(MALAYSIA)", 
		"モザンビーク共和国(MOZAMBIQUE)", 
		"ナミビア共和国(NAMIBIA)", 
		"ニューカレドニア(NEW CALEDONIA)", 
		"ニジェール共和国(NIGER)", 
		"ノーフォーク島(NORFOLK ISLAND)", 
		"ナイジェリア連邦共和国(NIGERIA)", 
		"ニカラグア共和国(NICARAGUA)", 
		"オランダ王国(NETHERLANDS)", 
		"ノルウェー王国(NORWAY)", 
		"ネパール王国(NEPAL)", 
		"ナウル共和国(NAURU)", 
		"ニウエ(NIUE)", 
		"ニュージーランド(NEW ZEALAND)", 
		"オマーン国(OMAN)", 
		"パナマ共和国(PANAMA)", 
		"ペルー共和国(PERU)", 
		"仏領ポリネシア(FRENCH POLYNESIA)", 
		"パプアニューギニア(PAPUA NEW GUINEA)", 
		"フィリピン共和国(PHILIPPINES)", 
		"パキスタン・イスラム共和国(PAKISTAN)", 
		"ポーランド共和国(POLAND)", 
		"サンピエール島・ミクロン島(SAINT PIERRE AND MIQUELON)", 
		"ピトケアン島(PITCAIRN)", 
		"プエルトリコ(PUERTO RICO)", 
		"ポルトガル共和国(PORTUGAL)", 
		"パラオ(PALAU)",
		"パラグアイ共和国(PARAGUAY)",
		"カタール国(QATAR)",
		"レユニオン(REUNION)",
		"ルーマニア(ROMANIA)",
		"ロシア連邦(RUSSIAN FEDERATION)",
		"ルワンダ共和国(RWANDA)",
		"サウジアラビア王国(SAUDI ARABIA)",
		"ソロモン諸島(SOLOMON ISLANDS)",
		"セイシェル共和国(SEYCHELLES)",
		"スーダン共和国(SUDAN)",
		"スウェーデン王国(SWEDEN)",
		"シンガポール共和国(SINGAPORE)",
		"セントヘレナ島(SAINT HELENA)",
		"スロベニア共和国(SLOVENIA)",
		"スバールバル諸島・ヤンマイエン島(SVALBARD AND JAN MAYEN)",
		"スロバキア共和国(SLOVAKIA)",
		"シエラレオネ共和国(SIERRA LEONE)",
		"サンマリノ共和国(SAN MARINO)",
		"セネガル共和国(SENEGAL)",
		"ソマリア民主共和国(SOMALIA)",
		"スリナム共和国(SURINAME)",
		"サントメ・プリンシペ民主共和国(SAO TOME AND PRINCIPE)",
		"エルサルバドル共和国(EL SALVADOR)",
		"シリア・アラブ共和国(SYRIAN ARAB REPUBLIC)",
		"スワジランド王国(SWAZILAND)",
		"タークス諸島・カイコス諸島(TURKS AND CAICOS ISLANDS)",
		"チャド共和国(CHAD)",
		"仏領極南諸島(FRENCH SOUTHERN TERRITORIES)",
		"トーゴ共和国(TOGO)",
		"タイ王国(THAILAND)",
		"タジキスタン共和国(TAJIKISTAN)",
		"トケラウ諸島(TOKELAU)",
		"トルクメニスタン(TURKMENISTAN)",
		"チュニジア共和国(TUNISIA)",
		"トンガ王国(TONGA)",
		"東チモール(EAST TIMOR)",
		"トルコ共和国(TURKEY)",
		"トリニダード・トバゴ共和国(TRINIDAD AND TOBAGO)",
		"ツバル(TUVALU)",
		"台湾(TAIWAN, PROVINCE OF CHINA)",
		"タンザニア連合共和国(TANZANIA, UNITED REPUBLIC OF)",
		"ウクライナ(UKRAINE)",
		"ウガンダ共和国(UGANDA)",
		"米領太平洋諸島(UNITED STATES MINOR OUTLYING ISLANDS)",
		"アメリカ合衆国(米国)(UNITED STATES)",
		"ウルグアイ東方共和国(URUGUAY)",
		"ウズベキスタン共和国(UZBEKISTAN)",
		"バチカン市国(VATICAN CITY STATE (HOLY SEE))",
		"セントビンセント及びグレナディーン諸島(SAINT VINCENT AND THE GRENADINES)",
		"ベネズエラ共和国(VENEZUELA)",
		"英領バージン諸島(VIRGIN ISLANDS (BRITISH))",
		"米領バージン諸島(VIRGIN ISLANDS (U.S.))",
		"ベトナム社会主義共和国(VIET NAM)",
		"バヌアツ共和国(VANUATU)",
		"ワリス・フテュナ諸島(WALLIS AND FUTUNA ISLANDS)",
		"西サモア(SAMOA)",
		"イエメン共和国(YEMEN)",
		"マイヨット島(MAYOTTE)",
		"(ユーゴスラビア連邦共和国)(YUGOSLAVIA)",
		"南アフリカ共和国(SOUTH AFRICA)",
		"ザンビア共和国(ZAMBIA)",
		"ザイール共和国(ZAIRE)",
		"ジンバブエ共和国(ZIMBABWE)"
	]
}