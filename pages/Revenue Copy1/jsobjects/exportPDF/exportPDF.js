export default {
	...
	createPDF: async () => {
		...
		await get_agency_info.run().catch((e) => showAlert('cannot get agency info','error'))
		...
		const customerDepartment= userInfo.department
		const customerName= userInfo.last_name + userInfo.first_name
		...
		const customerBankType = agencyInfo[0].classification || "";
	...
	tableArrayData: async () => {
		try{
			...
			let totalAmount = 0; // Khởi tạo biến để tích lũy tổng số tiền

			jsonData.forEach(item => {
				const proppertyName = `${item.building} ${item.room_number}`
				console.log('property', proppertyName, proppertyName.length)
				proppertyName.length >27 ? proppertyName.slice(0,26) +'...' : proppertyName;
				const row = [
					proppertyName,
					item.created_at,
					item.start_date,
